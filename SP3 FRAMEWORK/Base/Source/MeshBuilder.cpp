...
Mesh* MeshBuilder::GenerateQuad2(const std::string &meshName, Color color, float lengthX, float lengthY, TexCoord texCoord)
{
	std::vector<Vertex> vertex_buffer_data;
	std::vector<GLuint> index_buffer_data;
	Vertex v;
...
/*!
\brief
Generate the vertices of an obj;
Then generate the VBO/IBO and store them in Mesh object

\param meshName - name of mesh 
\param file_path - file path of the obj
...
/*!
\brief
Generate the vertices of the skyplane; Use random color for each vertex
Then generate the VBO/IBO and store them in Mesh object

\param meshName - name of mesh
\param color - Color of the skyplane
\param silces - Number of slices
\param PlanetRadius -
\param AtmosphereRadius -
\param hTile -
\param vTile -
...
Mesh* MeshBuilder::GenerateSkyplane(const std::string &meshName, Color color, int slices, float PlanetRadius, float AtmosphereRadius, float hTile, float vTile)
{
	...
	//Check number of slices (Long method)
	//(slices < 1) ? (slices = 1) : (slices > 256) ? (slices = 256) : (slices);

	//Check number of slices (Using Clamp)
	...
	for (int z = 0; z <= slices; ++z)
	{
		for (int x = 0; x <= slices; ++x)
		{
			...
			//tv.pos.Set(xDist, 0.0f - height, zDist);
			tv.pos.x = xDist;
...
/*!
\brief
Generate the vertices of the terrain; Use random color for each vertex
Then generate the VBO/IBO and store them in Mesh object
...
Mesh* MeshBuilder::GenerateMinimap(const std::string &meshName, Color color, float length)
{
	...
	v.color = color;
	v.normal.Set(0, 0, 1);
	v.texCoord.Set(2.0f, 0);
	vertex_buffer_data.push_back(v);

	v.pos.Set(0.5f * length, 0.5f * length, 0);
	v.color = color;
	v.normal.Set(0, 0, 1);
	v.texCoord.Set(2.0f, 2.0f);
	vertex_buffer_data.push_back(v);

	v.pos.Set(-0.5f * length, 0.5f * length, 0);
	v.color = color;
	v.normal.Set(0, 0, 1);
	v.texCoord.Set(0, 2.0f);
	vertex_buffer_data.push_back(v);
